...
SCENARIOS_FULL_OPTIMALITY = list(range(5, 41, 5))
...
def main():
    ...
    file_path_segment = "/random"

    # Group 1: Full Optimality
    for n in SCENARIOS_FULL_OPTIMALITY:
        total_n_value = n + 2
        for s in SAMPLES:
            file_name_segment = f"{total_n_value:02d}-{s:02d}"
            run_gams_scenario(n, total_n_value, s, reslim=86400, optcr=0.0, file_path=file_path_segment, file_name=file_name_segment)
    ...
    for n in SCENARIOS_TIME_LIMITED:
        ...
        for s in SAMPLES:
            file_name_segment = f"{total_n_value:02d}-{s:02d}"
            run_gams_scenario(n, total_n_value, s, reslim=3600, optcr=0.0, file_path=file_path_segment, file_name=file_name_segment)
